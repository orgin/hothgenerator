...
public class DagobahMushroomHutPopulator extends BlockPopulator
{
	...
	{
		if(this.schematics!=null && this.plugin.isGenerateMushroomHuts(world))
		{
			int rand = 1;
			int rarity = 3; 
	...
	private void placeMushroomHut(World world, Random random, Chunk chunk)
	{
		...
		if(biome.equals(Biome.MUSHROOM_ISLAND) || biome.equals(Biome.MUSHROOM_SHORE))  // Only in the correct biomes
		{
			...
			if(!block1.getType().equals(Material.MYCEL))
			{
				Block block2 = block1.getRelative(BlockFace.DOWN);
				if(block2.getType().equals(Material.MYCEL))
				{
					block = block1;
				}
				else
	...
	private int getSurfaceLevel(Block block)
	{
		...
		while(block.getY()>5 && !type.equals(Material.GRASS) && !type.equals(Material.DIRT) && !type.equals(Material.STATIONARY_WATER))
		{
