			...
			for(int i=0;i<parts.length;i++)
			{
				...
				if(schematic!=null)
				{
					...
					if(partType==PartType.LANDING || partType==PartType.MAIN)
					{
						...
						if((part.direction&0x8)==0)
						{
							world.getBlockAt(sxx+8, sy+17, szz).setType(Material.NETHER_FENCE);
						}
						if((part.direction&0x4)==0)
						{
							world.getBlockAt(sxx+16, sy+17, szz+8).setType(Material.NETHER_FENCE);
						}
						if((part.direction&0x2)==0)
						{
							world.getBlockAt(sxx+8, sy+17, szz+16).setType(Material.NETHER_FENCE);
						}
						if((part.direction&0x1)==0)
						{
							world.getBlockAt(sxx, sy+17, szz+8).setType(Material.NETHER_FENCE);
						}
					}
				}
			}
